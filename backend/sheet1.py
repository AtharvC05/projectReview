...
def determine_template_path(num_members, base_template_dir="pdf_template"):
    ...
    if num_members <= 4:
        template_path = os.path.join(base_template_dir, "Review-I-Sheet.pdf")
        template_type = "4_member"
    elif num_members == 5:
        template_path = os.path.join(base_template_dir, "Review-I-Sheet-5.pdf")
...
def generate_fillable_pdf(form_data, base_template_dir="pdf_template"):
    ...
    # Build field values dictionary (moved outside the date logic)
    field_values = {
    ...
    output_file = f"Review_1_{group_id}.pdf"
...
def get_responses(group_id):
    ...
    try:
        cursor.execute("SELECT field_name, field_value FROM responses WHERE group_id=%s", (group_id,))
