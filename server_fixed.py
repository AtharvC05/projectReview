from flask import Flask, render_template, redirect, request, jsonify, send_file, abort, session
...
# Import blueprints and authentication
...
@login_required
def index():
    # Serve review page based on role
...
@app.route('/review/<int:review_num>')  # Support both /review1 and /review/1
@login_required
...
@admin_required  # Only admins can generate PDF reports
...
def handle_pdf_generation(generate_function):
    ...
    # Allow both admin and regular users to generate PDFs from review forms
    if 'user_id' not in session:
        return jsonify({"error": "Authentication required"}), 401
    
    if session.get('role') not in ['admin', 'user']:
        return jsonify({"error": "Login required for PDF generation"}), 403
...
@admin_required  # Only admins can view PDF manager
def pdf_viewer():
    """Serve the PDF viewer webpage."""
    return render_template('pdf_viewer.html')

@app.route("/list-pdfs")
@admin_required  # Only admins can list PDFs
def list_pdfs():
...
@admin_required  # Only admins can download PDFs
...
@admin_required  # Only admins can view PDFs
...
@admin_required  # Only admins can delete PDFs
...
if __name__ == '__main__':
    ...
    print(f"   - Data Manager (Admin Only): /data-manager")
    print(f"   - Attendance Dashboard: /attendance-dashboard")
    print(f"   - PDF Viewer (Admin Only): /pdf_viewer")
    print(f"   - Scheduler (Admin Only): /scheduler")
    print("\n🔌 API Endpoints:")
    ...
    print(f"   User: evaluator1 / user123")
